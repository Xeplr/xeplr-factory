...
//   POST /factory/screens/:key/publish     draft → next version, and its table changed to match   { confirmDrop? }
...
function createFactoryRouter(stores, options) {
  ...
  router.post('/factory/screens/:key/publish', auth.design, handle(async function(req) {
    ...
    var result = await records.publish(req.params.key, { confirmDrop: confirmDrop }, req.user);
  ...
  router.get('/factory/records/:key/:id', auth.view, handle(async function(req) {
