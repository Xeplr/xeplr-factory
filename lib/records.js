...
function createRecordsStore(knex, screens, hooks, models) {
  ...
  async function get(screenKey, id, user) {
    ...
    try {
      ...
      var rows = (await ctx.query.limit(one ? 1 : LIST_LIMIT)).map(function(row) { return Model.fromDb(shapeRow(row, check.columns)); });
  ...
  async function save(screenKey, body, user) {
    ...
    try {
      ...
      if (id !== null) {
        var found = shapeRow(await readBack(scope(knex(r.table), r.table, cols), id), check.columns);
  ...
  async function remove(screenKey, id, user) {
    ...
    try {
      ...
      var before = shapeRow(await scope(knex(r.table).select(selectList(r.table, fields, columns)), r.table, cols).where(r.table + '.id', ctx.id).first(), columns);
