...
function createRecordsStore(knex, screens, hooks) {
  ...
  async function get(screenKey, id, user) {
    ...
    try {
      ...
      var rows = (await ctx.query.limit(one ? 1 : LIST_LIMIT)).map(function(row) { return shapeRow(row, check.columns); });
  ...
  async function save(screenKey, body, user) {
    ...
    try {
      ...
      if (id !== null) {
        ctx.previous = shapeRow(await readBack(scope(knex(r.table), r.table, cols), id), check.columns) || null;
      ...
      // The screen's own rules: types, required, lengths, patterns, options.
      var clean = applySchema(r.model.formSchema(r.doc), ctx.values, 'field');
      var row = {};
      ...
      ctx.id = savedId;
      var saved = shapeRow(await readBack(scope(knex(r.table), r.table, cols), savedId), check.columns);
      if (extra.length) {
        ...
        Object.assign(saved, shapeRow(more, check.columns));
      }
      ctx.result = saved;
    } catch (err) {
      await tellError(h, ctx, err);
  ...
  async function remove(screenKey, id, user) {
    ...
    try {
      ...
      ctx.previous = shapeRow(await scope(knex(r.table).select(selectList(r.table, fields, columns)), r.table, cols).where(r.table + '.id', ctx.id).first(), columns) || null;
      if (!ctx.previous) throw notFound('No ' + r.table + ' record "' + id + '"');
  ...
  return { get: get, list: list, save: save, remove: remove, options: options, allowedTables: allowedTables, publish: publish, describeTable: describeTable, columnsOf: columnsOf, forget: forget };
}
