...
function createRecordsStore(knex, screens, hooks, models) {
   ...
   *
   *   refused      a change that would lose or break data (narrowing, a foreign key moved) — 409
   *   won't fit    a field that changed kind (text → number…) where some saved values
   *                cannot become the new kind — 409, naming the column and showing up to
   *                five of the values in the way; nothing is changed
   *   to confirm   columns a removed field would drop, with how many values each holds,
   *                and columns changing kind, with how many values each converts — 409,
   *                until the same request comes back with confirmDrop / confirmConvert
  ...
  async function publish(screenKey, options, user) {
    ...
    var plan = model.planTableChange(current, doc, { managed: Array.from(managed), inUse: Array.from(inUse), confirmDrop: options.confirmDrop, convert: true, confirmConvert: options.confirmConvert });
    ...
    // A FIELD THAT CHANGED KIND. Every saved value must fit the new kind —
    // across every company, the column is shared. One that does not stops the
    // publish, with the values in the way; nothing is changed.
    var converts = [];
    var wontFit = [];
    // Older @xeplr/ui-factory models plan no conversions (they refuse instead).
    var planned = plan.convert || [];
    for (var v = 0; v < planned.length; v++) {
      var cv = planned[v];
      var failing = Number((await knex.raw(cv.check)).rows[0].n);
      if (failing) {
        var samples = (await knex.raw(cv.sample)).rows.map(function(r) { return r.v; });
        wontFit.push({ column: cv.column, from: cv.from, to: cv.to, failing: failing, samples: samples });
      } else {
        converts.push({ column: cv.column, from: cv.from, to: cv.to, records: Number((await knex.raw(cv.count)).rows[0].n) });
      }
    }
    if (wontFit.length) {
      var blocked = conflict('Cannot publish "' + screenKey + '" — ' + wontFit.map(function(w) {
        return '"' + w.column + '" has ' + w.failing + ' saved value' + (w.failing === 1 ? '' : 's') + ' that cannot become ' + w.to +
          ' (' + w.samples.map(function(x) { return JSON.stringify(x); }).join(', ') + (w.failing > w.samples.length ? ', …' : '') + ')';
      }).join('; ') + '. Correct those records, or change the field back.');
      blocked.body = { wontFit: wontFit };
      throw blocked;
    }

    if (plan.unconfirmed.length || (plan.unconfirmedConvert || []).length) {
      ...
      var parts = [];
      if (counts.length) parts.push('removes ' + counts.length + ' column' + (counts.length === 1 ? '' : 's') + ' from "' + table + '", with all their data: ' +
        counts.map(function(x) { return x.column + ' (' + x.records + ' value' + (x.records === 1 ? '' : 's') + ')'; }).join(', '));
      if (converts.length) parts.push('converts ' + converts.map(function(x) { return x.column + ' (' + x.from + ' → ' + x.to + ', ' + x.records + ' value' + (x.records === 1 ? '' : 's') + ')'; }).join(', '));
      var ask = conflict('Publishing ' + parts.join(', and ') + '. Confirm to continue.');
      ask.body = { confirm: counts, convert: converts, keep: plan.keep };
      throw ask;
    }
    ...
    try {
      ...
      forget(table);
      // 22: a value the database would not convert after all (out of range…).
      if (err.code && /^(42|23|22)/.test(err.code)) {
