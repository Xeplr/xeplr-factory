...
function createScreensStore(knex) {
  ...
  function shape(row) {
    ...
    return {
      ...
      mt: [row.mtId1, row.mtId2, row.mtId3, row.mtId4]
    };
  }
  ...
  /**
   * Every published version of every screen on a table, IN EVERY TENANT.
   *
   * Deliberately unscoped: a table is one table for all companies, so whether a
   * column may be dropped depends on screens this request cannot see. Used only
   * for that decision — nothing here is returned to a client.
   */
  async function publishedOnTableAllTenants(source) {
    var rows = await knex(TABLE).where({ source: source, status: 'published', isActive: true })
      .orderBy('screenKey').orderBy('version', 'desc');
    return rows.map(shape);
  }

  /** The draft becomes the next published version. Pass `trx` to publish inside a larger transaction. */
  async function publishDraft(screenKey, user, trx) {
    if (!trx) return knex.transaction(function(t) { return publishDraft(screenKey, user, t); });
  ...
  return { published: published, draft: draft, list: list, allPublished: allPublished, publishedOnTableAllTenants: publishedOnTableAllTenants, saveDraft: saveDraft, publishDraft: publishDraft };
}
